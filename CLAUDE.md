...
## Database Schema (`VerhuurFirmaDB`)
Migrated from PeterAllesweterdb to VerhuurFirmaDB. New Dutch table names:

```sql
Modellen          -- ModelID, Type, Merk, ModelNaam, AantalZitplaatsen, MotorInhoud
Locaties          -- LocatieID, Naam, Adres, Postcode, Gemeente, Telefoon
Voertuigen        -- VoertuigID, Nummerplaat, ModelID (FK), Kleur, Opties,
                  --   LocatieID (FK), Status (string), StatusID (FK),
                  --   StatusOpmerking, LaatsteStatusWijziging
VoertuigStatussen -- StatusID, StatusNaam, IsBeschikbaar, Volgorde, Actief
Klanten           -- KlantID, Voornaam, Achternaam, Email, Telefoon, Adres
Verhuringen       -- VerhuurID, VoertuigID (FK), KlantID (FK), LocatieID (FK),
                  --   StartDatum, EindDatum, DuurType, Prijs
-- Created in VerhuurFirmaDB to support chat/admin:
Conversations     -- conversationId, userId, timestamp
Messages          -- messageId, conversationId, role, content, timestamp, llmModelUsed
VehicleQueries    -- id, conversationId, vehicleId, queryType, response, timestamp
AdminUsers        -- id, name, email, passwordHash, role, isActive, forcePasswordReset, ...
```

**Key JOIN pattern** (vehicles):
```sql
...
**Availability check**: `s.IsBeschikbaar = 1` AND no overlapping Verhuringen with `StartDatum < @end AND EindDatum > @start`

**Note**: No pricing catalog — prices are per rental (Verhuringen.Prijs). Show "prijs op aanvraag" in UI.
